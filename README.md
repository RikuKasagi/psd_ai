...
├── setup.py               # パッケージ設定
├── generate_dataset.py     # データセット生成メインスクリプト
...
- **タイル分割**: 指定サイズ・オーバーラップでタイル分割（端はパディング）
- **データ分割**: train/val/testに指定比率で分割（シード固定で再現可能）
- **メタデータ**: manifestとログを自動生成
- **検証機能**: 入力・出力の整合性チェック
- **拡張性**: 将来の機能追加を考慮した設計

**処理フロー:**
1. PSDレイヤー受領（original, mask, refined）
2. マスク統合（優先度: mask > refined）
3. タイル分割（スケーリング + オーバーラップ対応）
...
**🚀 新機能**: データセット自動生成システムが追加されました！PSDレイヤーから機械学習用データセットを自動生成できます。
